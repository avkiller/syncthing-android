...
Use the import/export items in the app settings. This will use the folder `/sdcard/backups/syncthing/` for files. The actual files are stored in `/data/data/com.fireworld.syncthing/files/` which can only be accessed with root.
...
You can find the Syncthing logs in the app settings ("Debug" section). Additionally, logs are saved to the primary storage, under `Android/data/com.fireworld.syncthing/files/syncthing.log`.
...
- /storage/abcd-efgh/Android/com.fireworld.syncthing/files/syncthing.log *2)
